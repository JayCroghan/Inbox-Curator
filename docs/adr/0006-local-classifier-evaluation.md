...
Run one configured model profile at a time, one inference at a time. Persist each profile/item result, unload the active model before advancing, and resume without recomputing completed pairs. Use Ollama structured output with an actual JSON Schema, then validate the parsed domain result again in the application. Thinking content is never persisted.

Lock the versioned prompt before holdout can run. Treat high-confidence human KEEP → model UNWANTED on holdout as a failed safety gate. Do not automatically select a winner, classify unknown sources, or change human policy.
