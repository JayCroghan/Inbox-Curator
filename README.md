...
`MAIL-003A-PROMPT-V1` and `MAIL-003A-PROMPT-V2` remain immutable historical evidence. V2's Development + Validation run produced 32/32 canonical results for each of the five profiles, with 100% direct normalization, zero repair calls, and zero normalization failures. That established that the response architecture works and exposed an over-broad `NEEDS_REVIEW` decision policy instead. New development/validation runs therefore default to immutable `MAIL-003A-PROMPT-V3`, which refines KEEP/UNWANTED/NEEDS_REVIEW semantics while reusing the exact `MAIL-003A-OUTPUT-V2`, `NormalizeRepairV2`, and `MAIL-003A-REPAIR-V1` contracts. The input also derives unread, starred, important, and relationship percentages deterministically from frozen counts without changing the corpus.

The V2/V3 response protocol stores a rich primary explanation, tolerantly normalizes diagnostic fields, and makes at most one same-model extraction-only repair call when the recommendation itself is unusable. Repair supplies only the recommendation; confidence, category, reason codes, and explanation remain authoritative from the primary response, so repair can never elevate confidence. The complete repair response plus primary/repair prompt and schema versions/hashes are persisted. Every development run selects an explicit existing corpus; no corpus is silently substituted.

Runs execute as durable background jobs. Ollama HTTP requests have no implicit client timeout; the durable run cancellation token remains the active inference cancellation boundary. Before a fresh profile is measured, an already-resident exact model is unloaded, then every selected item—including any immediate repair call—is processed before the model is unloaded and the next profile begins. Primary and repair metrics are separate, and the first returned primary response records cold-load time even when normalization fails. Each item result is saved independently, restart resume skips completed profile/item pairs, cancellation is visible, and one invalid/failed result does not erase other work. The scoreboard distinguishes strict V1, normalize/repair V2, and policy-refined V3, and reports canonical coverage, direct/repair rates, unresolved normalization failures, semantic warnings, and repair overhead alongside judgment and performance metrics. Read-only inspection views cover disagreements/failures, self-repaired results, semantic warnings, normalization warnings, or all evaluated results—even when a result agrees with ground truth. Holdout execution is disabled and its examples remain hidden in MAIL-003A.2. The lab never chooses a winner, classifies unreviewed sources, or updates human policy.
